...
int tml_compare_msg(const void* a, const void* b) {
	const tml_message* msg_a = (const tml_message*)a;
	const tml_message* msg_b = (const tml_message*)b;
	return (msg_a->time > msg_b->time) - (msg_a->time < msg_b->time);
}
...
TMLDEF tml_message* tml_load(struct tml_stream* stream)
{
	...
	if (p.message_count)
	{
		...
		if (PrevMessage) PrevMessage->time = ~0 /*PrevMessage->next = TML_NULL*/;
	...
	qsort(messages, p.message_count, sizeof(*messages), tml_compare_msg);
